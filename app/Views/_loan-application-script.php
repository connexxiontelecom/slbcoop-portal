...
<script>
  ...
  $(document).ready(function () {
      ...
      if (loanType !== '' && loanType !== 'default') {
        $.ajax({
          ...
          success: function (response) {
            ...
            outstandingNLLoans = loanSetupDetails.outstanding_nl_loans
            ...
            if (outstandingNLLoans?.length) {
              $('#nl-loans-failed').attr('hidden', false)
            } else {
              $('#nl-loans-failed').attr('hidden', true)
            }
    ...
    $('form#loan-application').submit(function (e) {
      ...
      } else if (requiresGuarantor && (!guarantor1)) {
        Swal.fire("Invalid Submission", "Please fill in all required fields!", "error");
      } else if (outstandingNLLoans?.length) {
        Swal.fire("Invalid Submission", "You cannot apply for a Normal Loan with any other pending Normal Loan", "error");
      } else {
